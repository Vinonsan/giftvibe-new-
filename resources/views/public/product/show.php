...
<div data-product-detail class="mx-auto max-w-7xl">
    ...
    <div class="grid gap-10 lg:grid-cols-2 lg:gap-14">
        ...
        <div class="flex flex-col justify-center">
            ...
            <?php if($itemType==='product' && $productVariants):?><fieldset class="mt-6"><legend class="text-sm font-bold text-secondary">Choose colour <span class="text-rose-500">*</span></legend><div class="mt-3 flex flex-wrap gap-2"><?php foreach($productVariants as $index=>$variant):?><?php $variantPrice=$basePrice+(float)$variant['price_adjustment'];?><label class="cursor-pointer"><input class="peer sr-only" type="radio" name="product_variant" value="<?=(int)$variant['id']?>" data-variant-name="<?=htmlspecialchars((string)($variant['color_name']?:$variant['name']),ENT_QUOTES)?>" data-variant-price="<?=$variantPrice?>" data-variant-stock="<?=(int)$variant['stock_quantity']?>" <?=$index===0?'checked':''?>><span class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-secondary peer-checked:border-primary peer-checked:ring-2 peer-checked:ring-primary/20"><i class="h-5 w-5 rounded-full border border-black/10" style="background:<?=htmlspecialchars((string)($variant['color_hex']?:'#ffffff'))?>"></i><?=htmlspecialchars((string)($variant['color_name']?:$variant['name']))?> · LKR <?=number_format($variantPrice,2)?></span></label><?php endforeach;?></div></fieldset><?php endif;?>
...
function selectedVariant(){var input=document.querySelector('input[name="product_variant"]:checked');return input?{id:Number(input.value),name:input.dataset.variantName,price:Number(input.dataset.variantPrice)}:null;}
function saveToCart(product){var quantity=parseInt(document.getElementById('product-quantity').value,10)||1,variant=selectedVariant();if(<?=json_encode((bool)$productVariants)?>&&!variant){window.GiftVibeToast.show('Please select a colour.','error');return 0;}if(variant){product.variantId=variant.id;product.variantName=variant.name;product.price=variant.price;}var cart=JSON.parse(localStorage.getItem('giftvibe_cart')||'[]');var existing=cart.find(function(item){return item.slug===product.slug&&(item.type||'product')===(product.type||'product')&&Number(item.variantId||0)===Number(product.variantId||0);});if(existing)existing.qty=(Number(existing.qty)||1)+quantity;else cart.push(Object.assign({},product,{qty:quantity}));localStorage.setItem('giftvibe_cart',JSON.stringify(cart));if(window.updateCartUI)window.updateCartUI();return quantity;}
...
function detailBuyNow(slug,type){var quantity=parseInt(document.getElementById('product-quantity').value,10)||1,variant=selectedVariant();if(<?=json_encode((bool)$productVariants)?>&&!variant){window.GiftVibeToast.show('Please select a colour.','error');return;}window.giftRequireAuth('/checkout?'+(type==='combo'?'combo':'product')+'='+encodeURIComponent(slug)+'&qty='+quantity+(variant?'&variant='+variant.id:''));}
</script>
