    ...
    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
        ...
        <div>
            <span class="relative z-0 inline-flex -space-x-px rounded-md shadow-sm">
                <!-- Previous Button -->
                <a href="<?= $currentPage > 1 ? e($baseUrl . '?page=' . ($currentPage - 1)) : '#' ?>" class="relative inline-flex items-center rounded-l-md px-2 py-2 border border-slate-300 bg-white text-sm font-medium text-slate-500 hover:bg-slate-50 <?= $currentPage <= 1 ? 'pointer-events-none opacity-50' : '' ?>">
                ...
                ?>
                    <a href="<?= e($baseUrl . '?page=' . $i) ?>" class="relative inline-flex items-center border px-4 py-2 text-sm font-medium focus:z-20 <?= $pageClass ?>"><?= $i ?></a>
                <?php endfor; ?>

                <!-- Next Button -->
                <a href="<?= $currentPage < $totalPages ? e($baseUrl . '?page=' . ($currentPage + 1)) : '#' ?>" class="relative inline-flex items-center rounded-r-md px-2 py-2 border border-slate-300 bg-white text-sm font-medium text-slate-500 hover:bg-slate-50 <?= $currentPage >= $totalPages ? 'pointer-events-none opacity-50' : '' ?>">
