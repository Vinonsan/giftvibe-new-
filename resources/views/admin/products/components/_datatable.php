...
foreach ($products as $item) {
    ...
    $tableRows[] = [
        'product' => '<div class="flex items-center gap-3"><img src="' . htmlspecialchars($image, ENT_QUOTES) . '" class="h-12 w-14 shrink-0 rounded-lg object-cover"><div><p class="font-semibold leading-snug text-secondary">' . htmlspecialchars($item['name']) . '</p><p class="mt-0.5 text-[11px] text-secondary/60">' . htmlspecialchars($item['sku']) . ' · ' . htmlspecialchars((string) $item['category_names']) . '</p></div></div>',
...
$tableColumns = [
    ...
    ['key' => 'stock', 'label' => 'Stock', 'html' => true, 'type' => 'number'],
