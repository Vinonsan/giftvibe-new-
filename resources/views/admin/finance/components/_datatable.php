...
foreach($orderReports as $row){
 ...
 $meta=htmlspecialchars(json_encode(['kind'=>'order','reference'=>(string)$row['order_number'],'name'=>(string)$row['customer_name'],'date'=>(string)$row['date'],'sales'=>(float)$row['sales'],'cost'=>(float)$row['cost'],'profit'=>(float)$row['profit'],'amount'=>(float)$row['sales']],JSON_UNESCAPED_UNICODE),ENT_QUOTES);
 $tableRows[]=['name'=>'<strong class="text-secondary">'.htmlspecialchars((string)$row['customer_name']).'</strong>','type'=>'<span class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-700">Order cash in</span>','amount'=>'<strong class="text-emerald-700">+ LKR '.number_format((float)$row['sales'],2).'</strong>','date'=>'<span class="text-xs text-slate-500">'.htmlspecialchars((string)$row['date']).'</span>','year'=>$year,'month'=>$month,'_meta'=>$meta,'_actions'=>'<button type="button" data-drawer-open="finance-view-drawer" data-finance-view class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-secondary/60 hover:border-primary/30 hover:text-primary" title="View">'.$eye.'</button>'];
}
foreach($procurementReports as $row){
 $date=substr((string)$row['created_at'],0,10);[$year,$month]=$dateParts($date);
 $meta=htmlspecialchars(json_encode(['kind'=>'purchase','reference'=>(string)($row['sku']?:('PRODUCT-'.(int)$row['product_id'])),'name'=>(string)$row['name'],'date'=>$date,'unit_cost'=>(float)$row['unit_cost'],'quantity'=>(int)$row['quantity'],'amount'=>(float)$row['amount']],JSON_UNESCAPED_UNICODE),ENT_QUOTES);
