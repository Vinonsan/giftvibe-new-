<div id="purchase-auth-modal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-secondary/70 p-4" role="dialog" aria-modal="true" aria-labelledby="purchase-auth-title">
    <div class="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-primary/10 bg-white p-6 shadow-2xl sm:p-8">
        ...
        <form data-purchase-register class="mt-5 hidden space-y-4">
            ...
            <label class="block text-sm font-bold text-secondary">Email<input required type="email" name="email" autocomplete="email" class="mt-2 w-full rounded-xl border border-primary/20 px-4 py-3 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10"></label>
            <div class="grid gap-4 sm:grid-cols-2"><label class="block text-sm font-bold text-secondary">Phone number 1<input required type="tel" name="phone" autocomplete="tel" class="mt-2 w-full rounded-xl border border-primary/20 px-4 py-3 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10"></label><label class="block text-sm font-bold text-secondary">Phone number 2<input required type="tel" name="phone_2" class="mt-2 w-full rounded-xl border border-primary/20 px-4 py-3 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10"></label></div>
...
<script>
(function(){
    var modal=document.getElementById('purchase-auth-modal'),target='/shop',message=document.getElementById('purchase-auth-message');
    ...
    async function submit(form,url){var button=form.querySelector('button');button.disabled=true;button.classList.add('opacity-60');message.classList.add('hidden');try{var response=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(new FormData(form)))});var result=await response.json();if(result.success){window.location.href=target;return;}message.textContent=result.message||'Unable to continue.';message.classList.remove('hidden');}catch(error){message.textContent='Unable to continue. Please try again.';message.classList.remove('hidden');}finally{button.disabled=false;button.classList.remove('opacity-60');}}
    window.giftRequireAuth=async function(checkoutUrl){target=checkoutUrl;try{var response=await fetch('/api/auth/status',{headers:{'Accept':'application/json'}});var result=await response.json();if(result.logged_in){window.location.href=target;return;}}catch(error){}tab('login');open();};
    ...
    modal.querySelector('[data-purchase-login]').addEventListener('submit',function(event){event.preventDefault();submit(this,'/api/auth/login');});
    modal.querySelector('[data-purchase-register]').addEventListener('submit',function(event){event.preventDefault();submit(this,'/api/auth/register');});
